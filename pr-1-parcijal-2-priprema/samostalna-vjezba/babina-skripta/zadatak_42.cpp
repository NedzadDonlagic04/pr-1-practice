...
float* getSumOfSymmetricalPositionOnBoard(ReadOnlyFigure figure, const std::size_t size) {
    bool is00Black { !isSquareWhite(0, 0) }; 
    float * const sum { new float { 0.0f } };
    ...
    for (std::size_t i = 0; i < halfSize; i++) {
        for (std::size_t ii = is00Black; ii < size; ii += 2) {
            if (figure[i][ii].getVisinaFigure() == figure[size - i - 1][ii].getVisinaFigure()) {
